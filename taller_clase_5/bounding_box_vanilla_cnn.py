...
print('   - ENTRENAMIENTO COMPLETO: Todos los parámetros se entrenan')
...
def load_dataset():
    ...
    base_path = (
        ...
        'vision_aplicada/taller_clase_5/caltech-101'
    )
    categories = [
...
class VanillaBoundingBoxRegressor(nn.Module):
    def __init__(self, num_classes=4):
        super().__init__()

        # CNN OPTIMIZADA - Menos parámetros, mejor rendimiento
        self.features = nn.Sequential(
            ...
            # Bloque 5 - Global Average Pooling
            nn.Conv2d(256, 512, kernel_size=3, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((1, 1)),  # Salida 1x1 para menos parámetros
        )

        # Regresor OPTIMIZADO - Menos capas, mejor regularización
    ...
    def forward(self, x):
        ...
        # SIN SIGMOID - Permite valores fuera de [0,1] para mejor entrenamiento
        return coords
...
class WeightedMSELoss(nn.Module):
    def __init__(self, weights=None):
        super().__init__()
        if weights is None:
            # Pesos optimizados para mejor entrenamiento
            self.weights = torch.tensor([1.0, 1.0, 1.0, 1.0])  # Todos iguales
        else:
            self.weights = weights

    def forward(self, predictions, targets):
        # MSE con clipping para estabilidad
        predictions_clipped = torch.clamp(predictions, 0, 1)
        mse_per_coord = torch.mean((predictions_clipped - targets) ** 2, dim=0)
        ...
        # Penalización adicional para valores fuera de rango
        out_of_range_penalty = torch.mean(
            torch.relu(
                predictions - 1,
            ) + torch.relu(-predictions),
        )

        return weighted_mse + 0.1 * out_of_range_penalty
...
def train_model(
    ...
    scheduler, epochs, device, patience=15, min_delta=1e-6,
):
    train_losses = []
    val_losses = []
    ...
    print(
        f"🚀 Iniciando entrenamiento OPTIMIZADO de {MODEL_NAME} por "
        f"{epochs} épocas con early stopping (patience={patience})",
    )
    print('⚠️  Nota: CNN Vanilla requiere entrenamiento completo desde cero')
    print('=' * 80)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, targets) in enumerate(train_loader):
            ...
            if batch_idx % max(1, num_batches // 10) == 0:
                ...
                print(
                    f"\rEpoch {epoch+1}/{epochs} - Batch {batch_idx+1}/"
                    f"{num_batches} ({progress:.1f}%) - "
                    f"Loss: {loss.item():.4f}",
                    end='',
                )
